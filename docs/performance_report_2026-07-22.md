# Paper Performance Evaluation Report - 2026-07-22

## Run metadata

- Report version: `paper-performance-audit-v5`
- Evaluation timestamp: `2026-07-21T22:30:01Z` (`2026-07-22 02:30:01 Asia/Dubai`)
...
| --- | --- | --- |
| Same audit queued/running | Branch search found prior `audit/performance-evaluation-20260718`, `20260719`, `20260720`, and `20260721` branches; PR #21 is an open draft report-only PR. No dispatchable evaluator workflow was found on `main`. | No duplicate evaluator was started. |
...
| Real GitHub conclusion for this audit | No run was dispatched because the workflow is absent and Actions dispatch/list tools are unavailable. | No run link or conclusion exists for this audit. |
| Actions access in this environment | `gh` CLI is not installed; direct `git clone` failed with `CONNECT tunnel failed, response 403`; the exposed connector lacks Actions run-list, workflow-dispatch, artifact, cache, and log operations. | Blocked from authenticated Actions verification. |
...
| Telegram Test | #33 | schedule | Success; triggered July 16, 2026 15:10; duration 10s. This audit did not dispatch it. | https://github.com/260xs/trump-market-alert/actions/runs/29509943393 |

Limitation: the public Actions index showed 546 workflow runs and older scheduled workflow runs, but it did not expose authenticated logs/artifacts/caches or current queued/running state. Current `main` workflow files fetched through the connector are manual-only for the inspected scanner and Telegram-test workflows.

## Repository and data inventory

| Evidence source | Status | Audit impact |
| --- | --- | --- |
| Local checkout | Unavailable. Direct clone failed with proxy 403. | Could not run local evaluator, inspect SQLite files, run tests, or scan artifacts locally. |
...
| Prior audit artifacts | PR #21 contains a prior report-only performance audit, still draft/unmerged. | Context only; not production `main` evidence. |
...
| 4 | Actions dispatch/run-list unavailable in this environment | Prevents duplicate-run monitoring and real evaluator conclusion verification. | This run | High | No `gh`, direct GitHub clone 403, connector lacks Actions operations. |
...
- Added this report-only audit record on branch `audit/performance-evaluation-20260722`.
...
- Public Actions pages verified visible Success status for older public-figure run #251, hourly stock run #202, and Telegram Test run #33, but logs/artifacts/caches remain inaccessible without sign-in.
