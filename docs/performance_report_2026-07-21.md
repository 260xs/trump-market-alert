# Paper Performance Evaluation Report - 2026-07-21

## Run Metadata

- Report version: `paper-performance-audit-v4`
- Evaluation timestamp: `2026-07-20T22:30:01Z` (`2026-07-21 02:30:01 Asia/Dubai`)
- Repository inspected: `260xs/trump-market-alert`
- Base branch inspected: `main`
- Base commit observed: `291e135b8912ff4423f8f7bfd0392d631ac0c5df` (`Remove routine health cron Telegram ping`)
...
No matured High-confidence `Buy`, `Sell`, or verified public-figure `Good`/`Bad` alert could be evaluated in this run because timestamped production SQLite alert history, saved market data, strategy-version metadata, Actions caches, logs, and artifacts were not accessible to this automation. Missing records were excluded rather than reconstructed or guessed.
...
| Same audit queued/running | Open PRs #7, #12, and #16 are prior draft report branches, not queued/running GitHub Actions evaluations. No audit workflow was visible in the public Actions list. | No duplicate evaluator was started. |
| Actions dispatch/list capability | Direct `git clone` and `git ls-remote` failed with `CONNECT tunnel failed, response 403`; no `gh` CLI is installed; the exposed GitHub connector in this run did not include Actions run-list, artifact, cache, log, or workflow-dispatch tools. | Blocked. |

Recent public Actions evidence visible during this run:

| Workflow | Public run | Trigger | Verified visible conclusion | Link |
| --- | ---: | --- | --- | --- |
| Market-Moving Public Figure Alert | #251 | schedule | Listed on the public Actions index as a completed scheduled run with duration `1m 29s`. | https://github.com/260xs/trump-market-alert/actions/runs/29517005381 |
| Hourly Stock Research Scanner | #202 | schedule | Listed on the public Actions index as a completed scheduled run with duration `45s`. | https://github.com/260xs/trump-market-alert/actions/runs/29516399012 |
...
Important limitation: the public Actions page showed older scheduled runs, while current `main` workflow files fetched through the GitHub connector are manual-only `workflow_dispatch` for the inspected scanner/ops workflows. This report records the mismatch and does not infer current queued/running state from public HTML.
...
| Prior audit branches | PRs #7, #12, and #16 add report-only performance audits for prior dates; all remain draft/unmerged. | Useful context only; not production `main` state. |
...
| 5 | Workflow state mismatch | Can confuse audit scheduling and run attribution. | Current visible state | Medium | Public Actions shows older scheduled runs; current `main` scanner workflows fetched by connector are manual-only. |
...
- Checked prior audit PRs #7, #12, and #16 and confirmed they are draft report branches, not running evaluators.
- Added this report-only audit record on branch `audit/performance-evaluation-20260721`.
...
- Public Actions page showed the repository has 546 workflow runs and lists operational workflows, but no performance-evaluation workflow; the page did not expose authenticated queued/running state or logs.
