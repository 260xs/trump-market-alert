...
def test_public_figure_registry_candidates_are_shadow_only_and_have_sources():
    ...
    stale_ids = {entry["id"] for entry in registry.get("rejected_or_unavailable", []) if entry["status"] == "Stale"}

    assert "katharine_braddick" in candidate_ids
    assert "sam_woods" not in candidate_ids
    assert "sam_woods" in stale_ids
